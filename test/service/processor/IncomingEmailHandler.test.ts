...
describe('IncomingEmailProcessor tests', () => {
  ...
  test('Handle simple email', async () => {
    ...
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "janedoe", "john.doe@example.com", "test", new Date('2017-01-08T18:37:44.000Z'));
  });
  ...
  test('Handle complex email', async () => {
    ...
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "johndoe", "john.doe@example.com", "Nodemailer is unicode friendly ✔ (1476358788189)", new Date('2016-10-13T11:39:48.000Z'));
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "janedoe", "john.doe@example.com", "Nodemailer is unicode friendly ✔ (1476358788189)", new Date('2016-10-13T11:39:48.000Z'));
  });
  ...
  test('Handle cc email', async () => {
    ...
    // then:
    expect(emailDatabase.store).toHaveBeenCalledTimes(2);
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "janedoe", "john.doe@example.com", "hidden", new Date('2022-02-09T16:55:11.000Z'));
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "mariadoe", "john.doe@example.com", "hidden", new Date('2022-02-09T16:55:11.000Z'));
  });
  ...
  test('Handle bcc email', async () => {
    ...
    // then:
    expect(emailDatabase.store).toHaveBeenCalledTimes(1);
    expect(emailDatabase.store).toHaveBeenCalledWith("message-id", "hidden", "john.doe@example.com", "hidden", new Date('2022-02-09T16:55:11.000Z'));
  });
});
